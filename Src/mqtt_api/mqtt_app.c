...
void MqttThread( void ){
    ...
    if( MQTT_TASK_HANDLE == NULL ){
      MQTT_INFO("create MQTT thread failed\r\n");
      while( true );
    }
        ...
        if( MqttSendFlag() ){
            ...
            if( xQueueReceive( MQTT_SEND_QUEUE_HANDLE, &( MqttSend ), ( TickType_t ) 0 ) ){
                ...
                if( MqttEncryption( &MqttSend, &(LTE_SEND_PARA) )){
                    _info_( "mqtt", (uint8_t *)LTE_SEND_PARA.HANDLE, LTE_SEND_PARA.LEN );
                    transport_sendPacketBuffer( MQTT_SOCKET_HANDLE, ( uint8_t *)(LTE_SEND_PARA.HANDLE), LTE_SEND_PARA.LEN );
                }
            }
        }
    }else{
...
//						commandProcessing( payload_in, payloadlen_in );
			...
			default:
                ...
                {
                    if( mqttmsgType != ( enum msgTypes )(-1) ){
